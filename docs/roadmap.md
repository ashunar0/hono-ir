...
## 実装状況 (2026-05-06 時点、Tags + Optimistic UI → RealWorld spec 機能完成 + 楽観的更新)
...
- **`TagPill` component**: ArticleCard / Show / TagInput で tag pill style が 3 箇所重複しているので括る候補
- **server.ts の Home route 切り出し**: 複数 page で articles を出す機会が増えたら page-props 組み立て関数を articles feature 側に切り出す候補

## 次回への引き継ぎ

### 状態
- main: 直前 push 済みの `77b04e4` (Tags 引き継ぎメモ) の上に、Optimistic UI (Favorite / Follow) + `visit` ラッパー + 引き継ぎメモ更新を本セッションで commit 予定。push 後の最新 commit hash は次セッションで `git log --oneline -3` で確認
- typecheck / build OK、Playwright で動作確認実施済み:
  - Home の Favorite ボタン: ♡ 0 → ♥ 1 (POST 303) ✓ / ♥ 1 → ♡ 0 (DELETE 303) ✓
  - Profile (test01) の Follow ボタン: Follow → Unfollow (POST 303) ✓ / Unfollow → Follow (DELETE 303) ✓
  - visit ラッパー refactor 後も同じ動作維持 ✓
...
H4 → H5 → H6 → B → C が筋 (フロント側で Optimistic + deferred + mergeProps を一通り体験 → Phase 3 PR で総仕上げ → 404 は仕上げ)。D は気になったら都度。
