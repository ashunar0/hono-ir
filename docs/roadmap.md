...
## 実装状況 (2026-05-06 時点、Tags + Optimistic UI → RealWorld spec 機能完成 + 楽観的更新 + UI 整え + Comment optimistic + 404 Inertia 化 + page-props builder + c.back/c.forward 対称 API + Deferred props (H5))
...
- [x] **c.forward を新設して c.back との対称 API に (D2 続き)**: `c.back({ flash, fallback })` は「referer 動的 redirect + flash」のプリミティブに進化していたのに対し、残り 7 箇所の `setFlash + c.redirect(url, 303)` (auth signup/login/logout、articles create/update/delete、users settings) は依然 2 行のままだった。`c.forward(url, { flash, errors })` を inertia-helpers.ts に追加して 1 行化、c.back と対称な API に揃える (動的 referer = c.back / 静的 URL = c.forward)。flash payload 処理は `applyFlashPayload` を共通化。**副作用**: `setFlash` の direct 利用が library 内部 1 箇所のみに集約。Phase 3 PR の adapter 側で c.back / c.forward の対称ペア提案の素材完成。Rails の `redirect_to` / `redirect_back_or_to` と整合
- [x] **D3. arbitrary value (色味付き) を Tailwind preset に寄せる**: 前 wave (4885a0a) で gray scale + 寸法は preset 化済み、残り Bootstrap flavor の Flash (alert green/red)、Material flavor の Like ♥ 色、`rounded-[4px]`、TagPill の `border-[#bbb]` / `px-[0.6rem]` `py-[0.1rem]` 等を寄せた。色は近似 preset (Bootstrap `#D4EDDA` ≒ Tailwind `green-100`、`#F8D7DA` ≒ `red-100`) で視覚的 regression 最小限。残し: `w-[100px] h-[100px]` (Profile avatar、preset 24/28 の中間) のみ意図的
- [x] **Deferred props (H5)**: Inertia 2.0 の `<Deferred>` を `@hono/inertia` 0.1.0 に layered で乗せる user-land 実装。`src/lib/inertia-defer.ts` に `defer(cb, group?)` helper + `inertiaDeferred()` middleware (sharedData と同形の c.render ラップ pattern)。class instance sentinel + post-process (JSON parse 詰め直し / HTML 埋め込み script を regex 置換) で page object の top-level に `deferredProps: { groupName: [keys] }` を注入。適用: Show の comments + Home の popularTags。Home の aside は子 component PopularTagsAside に抽出 (inline `popularTags.length` access が fallback render 時に undefined crash するので公式 PermissionsChildComponent pattern 踏襲)。defer × useOptimistic 共存確認済 (Comment add/delete optimistic は partial reload `only=["comments","flash"]` で defer callback 評価 + flash 注入が両立)。Phase 3 PR 素材: adapter 取り込み時に regex fragility 自然解消、`server.ts` の `app.use(inertiaDeferred())` 1 行が消える
...
### 状態
- main: 本セッションで D3 完了 (色味付き preset 寄せ) + H5 (Deferred props) + docs 引き継ぎ の 3 commit を追加 + push 済み。最新 commit hash は次セッションで `git log --oneline -10` で確認
- typecheck OK、Playwright + curl で動作確認実施済み:
  - 初回 HTML: `props` から defer 対象除外、`deferredProps: { default: ["popularTags" / "comments"] }` が page object top-level に注入
  - mount 後の自動 partial reload: defer callback 評価され通常 props として返る、`deferredProps` field は省略
  - defer 非要求の partial reload (例: `only=["article"]`): callback skip (server work 省く)
  - Comment optimistic add/delete (`only=["comments","flash"]`) と完全共存: defer callback 評価 + flash 注入が両立、useOptimistic の `id<0` pending 判定にも干渉なし
...
- **dev server は localhost:5173 で起動中** (前セッション終了後に再起動して 5173 が空いた)。次セッション開始前に `bun run dev` の状態確認 → 動いてなければ再起動
...
#### 2026-05-06 (続続続続続続続: c.forward 新設で c.back と対称 API に)

D2 の続き。c.back の進化を捕まえてからの「c.redirect 側にも対称な flash 込み API ほしくない?」という議論から派生。

- **c.back の 3 段階の進化**: form error 専用 → referer 動的 redirect → fallback 活用 (D2)。気付かないうちに「referer or fallback への 303 redirect + 任意 flash」というプリミティブに育ってた
- **対する setFlash + c.redirect(url, 303)**: 7 箇所 (auth 3 + articles 3 + users 1) が 2 行慣用句のまま残っていた
- **c.forward を新設**: `c.forward(url, { flash, errors })` で 1 行化、c.back との対称 API に。flash payload 処理は `applyFlashPayload` で共通化
- **動的 vs 静的の違い**: c.back は referer 動的、c.forward は呼び元が URL 静的指定。形は近いが意味論は違う、別関数で意図を分ける (Rails / Phoenix / Laravel の前例とも整合)
- **副作用**: `setFlash` の direct 利用が library 内部 (inertia-helpers.ts) 1 箇所のみに集約。features 側からは消滅
- **Phase 3 PR 素材**: adapter (`@hono/inertia`) に c.back / c.forward の対称ペアを提案する素材完成

判断記録の追加分は `docs/decisions.md` の 2026-05-06 セクション末尾「c.forward を新設して c.back との対称 API に (D2 続き)」参照。

#### 2026-05-06 (続続続続続続続続: H5 Deferred props を user-land 実装 + D3 完了)

c.back/c.forward 整理から話題転回、roadmap の H5 (Inertia 2 Deferred props) に着手。「Hono + Inertia + React らしい」選択肢として H5 / H6 / H7 から H5 を選んだ (RSC streaming / Astro Server Islands と同じ問題を Inertia 流に解いた API、`useOptimistic` 入れた直後だと「server stream × client optimistic」の対比で層の違いが見える)。あわせて D3 (色味付き arbitrary value の preset 寄せ) の残りも片付けた。

- **設計の選び方**: A. inertia() 自体を user-land で fork / B. layered post-processing (新 middleware で response 後加工) / C. sharedData に同居、を提示。ユーザー側から「PR 寄せ視点で fork (A) しないで、まず project 内で動くもの user-land で作って、出来栄え次第で adapter 強化路線に乗せる」という順序整理 → **B 採用**。sharedData / c.back / c.forward と同じ進め方
- **server.ts への影響は 1 行 + import 1 行**: `app.use(inertiaDeferred())` を sharedData の後ろに追加するだけ。defer() helper + inertiaDeferred() middleware は同じ file (`src/lib/inertia-defer.ts`) に同居
- **仕様の壁: deferredProps は page object の top-level field**: sharedData が拡張する `auth` / `flash` / `errors` は page.props 配下なので merge で済むが、Inertia 2 protocol は `deferredProps` を **page object のトップレベル**に置く (`@inertiajs/react` の `<Deferred>` がそこを読む)。@hono/inertia 0.1.0 の renderer は page object を `{ component, props, url, version }` 4 field 固定で組む → 注入の余地なし → original を呼んだ後の Response を後加工 (JSON は parse → 詰め直し、HTML は埋め込み `<script data-page="app">` の regex 置換)
- **Sentinel は class instance**: `defer<T>(cb, group?): T` は `DeferredValue` のインスタンスを返す。sharedData の deepMerge (isPlainObject ガード) が拾わないので key 衝突しても素通り。型は T を透過、service / route 側は通常 props と同じ型として書ける
- **partial reload 評価ロジック**: `X-Inertia-Partial-Data` header に該当 key が含まれる時のみ callback 評価。defer 非要求の partial reload (Pagination の `["articles","articlesCount","query"]` 等) は callback skip で server work 省く
- **適用: Show の comments + Home の popularTags**:
  - Show: article のみ即時 await、comments は `defer(async () => listComments(db, slug).then(...))` で sentinel 化、`<Deferred data="comments" fallback="Loading comments...">` で囲む
  - Home: `loadHomePage` で popularTags を `defer(() => listAllTags(db))` (Promise.all から外して articles だけ即時)、`<Deferred data="popularTags" fallback={<aside>Loading...</aside>}>` で囲む
- **inline access の crash と子 component 抽出 pattern**: Home.tsx で `<Deferred>` 子に inline で `popularTags.length > 0 && (<aside>...</aside>)` を書いたら fallback render 時 (popularTags = undefined) に `.length` 触って crash。JSX 子要素は親 render で eagerly 評価される (Deferred が描画切替判断するのは「描画するかどうか」だけ、子 element の構築自体は走る)。**公式 docs の `<PermissionsChildComponent />` pattern** 踏襲、子 component (PopularTagsAside) に抽出。子は `<Deferred>` が children を mount するときだけ function call → tags は必ず string[]。Show.tsx の CommentsSection は元から子 component だったので無事だった
- **defer × useOptimistic 共存確認**: Comment add/delete (`only=["comments","flash"]`) は partial reload で defer callback 評価 + flash 注入が両立。useOptimistic の `id<0` pending 判定にも干渉なし。「層が直交してる」感が出た (defer = 「いつ load するか」、useOptimistic = 「往復中の体感」)
- **Phase 3 PR 素材**: adapter (`@hono/inertia`) に取り込まれた未来予想図:
  - defer 単独吸収: `import { defer } from "@hono/inertia"` で `app.use(inertiaDeferred())` の 1 行が消える、`src/lib/inertia-defer.ts` ファイルごと削除可
...
- **D3 (色味付き preset 寄せ)**: 前 wave (4885a0a) で gray scale + 寸法は preset 化済み、残り Bootstrap flavor の Flash (alert green/red)、Material flavor の Like ♥ 色、`rounded-[4px]`、TagPill の `border-[#bbb]` / `px-[0.6rem]` `py-[0.1rem]` を寄せて完了。色は近似 preset で視覚的 regression 最小限

判断記録の追加分は `docs/decisions.md` の 2026-05-06 セクション末尾「Inertia 2 deferred props を user-land で実装 (H5)」参照。

### 次セッション最初の一手

RealWorld spec 機能 + Favorite/Follow Optimistic + Comment Optimistic + 404 Inertia 化 + page-props builder (D1) + c.back/c.forward 対称 API (D2 + D2 続き) + **Deferred props (H5)** + **D3 (色味付き preset 寄せ)** まで完成 ✓。残る選択肢:

**H. フロント rendering 制御の続き** ← Optimistic UI / Deferred の延長線
- **H6. `mergeProps` で infinite scroll** — Pagination を「Load more」ボタン + 配列 append 型に。TanStack の `useInfiniteQuery` 的体験を Inertia で表現。H5 と同じく Inertia 2 specific 機能で、defer 後の自然な続き
- **H7. `@inertiajs/progress`** — グローバル progress bar、1 行で入る (defer の partial reload にも自動で乗る)
- **H9. tab 切替 / pagination 中の skeleton** — partial reload 中の白画面を skeleton で埋める

**B. Phase 3: upstream PR** ← 本来の集大成
- 整理し直した PR target (`docs/decisions.md` 「大物 (将来計画)」参照): **4 PR / 2 repo** (defer が増えた)
  ...
  - `src/lib/inertia-helpers.ts` (c.back / c.forward) → `honojs/middleware` (`@hono/inertia`)
  - `src/lib/inertia-defer.ts` (defer + inertiaDeferred) → `honojs/middleware` (`@hono/inertia`)
- 順序: 先に `@hono/inertia` 側 3 PR で実績作る → 後で本体に Promise router 提案
- 動作確認は Articles / Comments / Favorites / Tags / Optimistic UI / Deferred 全機能で十分積んだ
- React 19 普及期 + Inertia 2 浸透期の今がタイミング◎
...
**筋の良い順**: H6 (defer の延長 = mergeProps による server-driven infinite scroll) → B (Phase 3 PR) が自然。`@inertiajs/core` への visit Promise wrapper は React 19 + Optimistic UI + Deferred 全部使った後だと説得力が最大化される。E は「見せたい」モードに切り替わったら。
