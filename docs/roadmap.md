...
## 実装状況 (2026-05-06 時点、Tags + Optimistic UI → RealWorld spec 機能完成 + 楽観的更新 + UI 整え + Comment optimistic + 404 Inertia 化 + page-props builder + c.back/c.forward 対称 API)
...
RealWorld spec 機能 + Favorite/Follow Optimistic + Comment Optimistic + 404 Inertia 化 + page-props builder 切り出し (D1) + c.back/c.forward 対称 API (D2 + D2 続き) まで完成 ✓。残る選択肢:
